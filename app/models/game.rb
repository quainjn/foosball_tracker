...
class Game < ActiveRecord::Base
  ...
  def push_to_ducksboard
    user_ranks = League.order("games_count desc").first.users.sort_by(&:current_rank).reverse.collect{|user| {name: user.name, values: ["#{user.wins_count} | #{user.losses_count}", "#{'%0.1f' % user.current_rank}"] }}
